...
st.markdown("""
    ...
    .header {
        text-align: center;
        padding: 20px;
        background: url('D:\IIITP\7thSem\AIAdaptive\AI-Powered-Adaptive-Learning-Platform\Random Input Vector (4).png') no-repeat center center;
...
#             st.error("Please enter a sentence to translate.")
    ...
    for speaker, text in st.session_state['conversation']:
        if speaker == "You":
            ...
            st.markdown(f"""
            <div style='text-align: left; background-color: #f50000;color: black; padding: 8px; border-radius: 10px; margin: 5px;'>
                <b>Bot:</b> {text}
            </div>
            """, unsafe_allow_html=True)
    ...
    API_KEY="AIzaSyCvJICfDlFF8InL_zqGeR_uaJPl3WhjjNI"
    genai.configure(api_key=API_KEY)

    model = genai.GenerativeModel('gemini-pro')
    chat = model.start_chat(history=[])

    instruction = "In this chat, respond as if you're explaining things to a twelve-year-old child."
    # instruction ="In this chat,first ask the user's age,wait for his response and then reply as if you're explaining things to a person of that age"
